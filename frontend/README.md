...
1. **Sign Up**: User creates account → Backend returns JWT token → Token stored in localStorage → Redirect to dashboard
2. **Log In**: User authenticates → Backend returns JWT token → Token stored in localStorage → Redirect to dashboard
3. **Dashboard**: Protected route checks for token → Fetches user data from `/auth/me` → Displays user info
4. **Logout**: Removes token from localStorage → Redirects to login page
...
**Important:** This frontend requires the backend API to be running. Make sure to set up and start the backend project first.
